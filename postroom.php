...
if (isset($_GET["getroom"]) && !empty($_GET["getroom"])) {
    ...
    if ($_SESSION["UserType"] == "Admin") {
        $sql = 'SELECT * from tbl_room and Status = 1 order by CDate desc';
        $stmt = $conn->prepare($sql);
    } else if ($_SESSION["UserType"] == "Staff") {
        $sql = 'SELECT * from tbl_room where UserId = :UserId and Status = 1 order by CDate desc';
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':UserId', $UserId, PDO::PARAM_INT);
    } else {
        $sql = 'SELECT * from tbl_room where RoomId in (select RoomId from tbl_assign where UserId = :UserId) and Status = 1 order by CDate desc';
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':UserId', $UserId, PDO::PARAM_INT);
    }
    try {
